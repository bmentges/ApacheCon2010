			...
			j_frames.each(function(i){
				...
				/*
					...
					$('a', this).click(function(e){
						// Get the current position of the gallery & the clicked frame
						ms_posGallery = getPos(j_gallery[0]);
						ms_posFrame = getPos(j_frames[i]);

						ms_posY = ms_posGallery.top;
						ms_posX = ms_posGallery.left + ms_posFrame.left;

						// Show the caption box
						//$("#carousel-caption").show().css({'top': e.pageY, 'left': e.pageX});
						$("#carousel-caption").show().css({'top' : ms_posY, 'left' : ms_posX, 'width' : '140px', 'height' : '140px', 'opacity' : 0.90});

						//paused = true;
						//$(document).stopTime("transition");

						// Set the contents of the caption box
						ms_html = "<a href=\"" + this.href + "\">" + this.title + "</" + "a>";
						ms_html += "<a href=\"" + this.href + "\" class=\"button\">search</" + "a>";
						$("#carousel-caption .title").html(ms_html);
